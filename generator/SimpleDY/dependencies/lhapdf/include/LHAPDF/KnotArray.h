...
#include <algorithm>  // for std::upper_bound, std::find
...
namespace {
  ...
  // PERFORMANCE-CRITICAL: This function is very hot. Keep optimized for speed. When upgrading to C++20, add [[unlikely]] to the edge case branch.
  inline size_t indexbelow(double value, const std::vector<double>& knots) {
    const size_t n = knots.size();
    const double* b = knots.data();
    const double* e = b + n;

    size_t i = std::upper_bound(b, e, value) - b;
    if (i >= n) i = n - 1;
    return i - 1;
  }
  ...
  int findPidInPids(int pid, const std::vector<int>& pids) {
    ...
    if (it == pids.end())
      ...
      // save to cast, given small number of pid's
      return static_cast<int>(std::distance(pids.begin(), it));
  }
...
namespace LHAPDF {
  ...
  public:
    ...
    /// How many flavours are stored in the grid
    size_t numflavs() const { return _shape.back(); }
    ...
    inline size_t ixbelow(double x) const { return indexbelow(x, _xs); }

    /// Find the largest grid index below given q2, such that q2knots[index] < q2
    inline size_t iq2below(double q2) const { return indexbelow(q2, _q2s); }
    ...
    /// @brief Convenient accessor to the polynomial coefficients
    ///
    /// @note Returns reference rather than value, to be able to read multiple adjacent coeffs at once.
    ...
    int lookUpPid(size_t id) const { return _lookup[id]; }

    /// Set of x knots
    double xs(size_t id) const { return _xs[id]; }

    /// Set of log10(x) knots
    double logxs(size_t id) const { return _logxs[id]; }

    /// Set of Q2 knots
    double q2s(size_t id) const { return _q2s[id]; }

    /// Set of log10(Q2) knots
    double logq2s(size_t id) const { return _logq2s[id]; }

    /// @brief Shape of the interpolation grid
    ///
    /// @todo Document/explain better!
    size_t shape(size_t id) const { return _shape[id]; }
    ...
    /// @todo Document and normalise naming style
    ...
    /// @name Const accessors to the internal data container
    /// @{
    ...
    /// @name Non const accessors for programmatic filling
    /// @{
